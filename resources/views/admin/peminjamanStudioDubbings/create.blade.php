...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.peminjaman-studio-dubbings.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\PeminjamanStudioDubbing::OPERATOR_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('operator') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="operator_{{ $key }}" name="operator" value="{{ $key }}" {{ old('operator', 'tidak') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('persetujuan') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="persetujuan" id="persetujuan" value="1" required {{ old('persetujuan', 0) == 1 ? 'checked' : '' }}>
                    <label class="required form-check-label" for="persetujuan">Dengan ini saya bertanggung jawab atas kualitas dan kuantitas sarana dan prasarana pda Studio Dubbing Selama Masa Peminjaman. Dan Bersedia untuk Bertanggung Jawab apabila ada kerusakan/kehilangan</label>
                </div>
