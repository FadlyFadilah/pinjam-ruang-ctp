...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.studioFoto.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.studio-fotos.update", [$studioFoto->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="pemohon">{{ trans('cruds.studioFoto.fields.pemohon') }}</label>
                <input class="form-control {{ $errors->has('pemohon') ? 'is-invalid' : '' }}" type="text" name="pemohon" id="pemohon" value="{{ old('pemohon', $studioFoto->pemohon) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('alamat') ? 'is-invalid' : '' }}" name="alamat" id="alamat" required>{{ old('alamat', $studioFoto->alamat) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('wa') ? 'is-invalid' : '' }}" type="text" name="wa" id="wa" value="{{ old('wa', $studioFoto->wa) }}" required>
                @if($errors->has('wa'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('produk') ? 'is-invalid' : '' }}" type="text" name="produk" id="produk" value="{{ old('produk', $studioFoto->produk) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('profil') ? 'is-invalid' : '' }}" name="profil" id="profil" required>{{ old('profil', $studioFoto->profil) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('konten') ? 'is-invalid' : '' }}" name="konten" id="konten" required>
                    ...
                    @foreach(App\Models\StudioFoto::KONTEN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('konten', $studioFoto->konten) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('oss') ? 'is-invalid' : '' }}" type="text" name="oss" id="oss" value="{{ old('oss', $studioFoto->oss) }}" required>
                @if($errors->has('oss'))
