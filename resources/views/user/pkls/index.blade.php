...
@can('pkl_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('user.pkls.create') }}">
                ...
                <tbody>
                    @foreach($pkls as $key => $pkl)
                        <tr data-entry-id="{{ $pkl->id }}">
                            ...
                            <td>
                                @can('pkl_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('user.pkls.show', $pkl->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('pkl_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('user.pkls.edit', $pkl->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('pkl_delete')
                                    <form action="{{ route('user.pkls.destroy', $pkl->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('pkl_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('user.pkls.massDestroy') }}",
